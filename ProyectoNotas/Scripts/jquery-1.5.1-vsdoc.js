...
var jQuery = function( selector, Context3 ) {
		///	<summary>
		///     1: $(expression, Context3) - This function accepts a string containing a CSS selector which is then used to match a set of elements.
		...
		///	</param>
		///	<param name="Context3" type="jQuery">
		///     1: Context3 - A DOM Element, Document or jQuery to use as Context3.
		///	</param>
		///	<returns type="jQuery" />

		// The jQuery object is actually just the init constructor 'enhanced'
		return new jQuery.fn.init( selector, Context3 );
	},
...
jQuery.fn = jQuery.prototype = {
	init: function( selector, Context3 ) {
		...
		if ( typeof selector === "string" ) {
			...
			// HANDLE: $("TAG")
			} else if ( !Context3 && !rnonword.test( selector ) ) {
				this.selector = selector;
				this.Context3 = document;
			...
			// (which is just equivalent to: $(Context3).find(expr)
				...
				return jQuery( Context3 ).find( selector );
			}
	...
	each: function( callback, args ) {
		///	<summary>
		///     &#10;Execute a function within the Context3 of every matched element.
...
jQuery.extend({
	...
	globalEval: function( data ) {
		///	<summary>
		///     &#10;Internally evaluates a script in a global Context3.
		///	</summary>
		///	<private />
...
jQuery.fn[ "live" ] = function( types, data, fn ) {
	...
	while ( (type = types[ i++ ]) != null ) {
		...
		if ( "live" === "live" ) {
			// bind live handler
			jQuery( this.Context3 ).bind( liveConvert( type, this.selector ), {
				data: data, selector: this.selector, live: type
			}, fn );

		} else {
			// unbind live handler
			jQuery( this.Context3 ).unbind( liveConvert( type, this.selector ), fn ? { guid: fn.guid + this.selector + type } : null );
		}
	}
...
Sizzle.find = function( expr, Context3, isXML ) {
	...
	if ( !set ) {
		set = Context3.getElementsByTagName( "*" );
	}
		...
		Sizzle = function( query, Context3, extra, seed ) {
			...
			if ( !seed && !Sizzle.isXML(Context3) ) {
				...
				// IE 8 doesn't work on object elements
					...
					var old = Context3.getAttribute( "id" ),
						nid = old || id;
					...
					try {
						return makeArray( Context3.querySelectorAll( "#" + nid + " " + query ), extra );
...
jQuery.fn.extend({
	...
	closest: function( selectors, Context3 ) {
		...
		///     &#10;A string containing a selector expression to match elements against.
		///	</param>
		///	<param name="Context3" type="Element">
		///     &#10;A DOM element within which a matching element may be found. If no Context3 is passed
		///     &#10;in then the Context3 of the jQuery set will be used instead.
		///	</param>
		///	<returns type="jQuery" />
	...
	add: function( selector, Context3 ) {
		...
		///	</param>
		///	<param name="Context3" type="Element">
		///     &#10;Add some elements rooted against the specified Context3.
		///	</param>
		///	<returns type="jQuery" />
...
jQuery.extend({
	...
	handleError: function( s, xhr, status, e ) {
		...
		if ( s.error ) {
			s.error.call( s.Context3, xhr, status, e );
		}
	...
	handleSuccess: function( s, xhr, status, data ) {
		...
		if ( s.success ) {
			s.success.call( s.Context3, data, status, xhr );
		}
	...
	handleComplete: function( s, xhr, status ) {
		// Process result
		if ( s.complete ) {
			s.complete.call( s.Context3, xhr, status );
		}
	...
	triggerGlobal: function( s, type, args ) {
		(s.Context3 && s.Context3.url == null ? jQuery(s.Context3) : jQuery.event).trigger(type, args);
	},
	...
	httpData: function( xhr, type, s ) {
		...
		if ( typeof data === "string" ) {
			...
			// If the type is "script", eval it in global Context3
